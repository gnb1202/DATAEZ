...
- **2단계 확인 플로우와 undo 저널.** 현재 프롬프트는 확인 질문을 금지하고
  ...
  가야 하는 작업이라 분리했습니다.
- **모델 ID 미검증.** `gpt-5.4` / `gpt-5.4-nano`가 유효한지 실제 키로 확인하지
  않았습니다. 티어 교체의 비용·품질 효과는 따라서 **아직 측정되지 않았습니다.**
  `make eval`이 실제 키가 있을 때 그 수치를 만듭니다.
...
- **278 tests**, 19개 모듈. 외부 의존성(DB·OpenAI·Redis·S3) 전부 mock — 인프라 없이 실행
