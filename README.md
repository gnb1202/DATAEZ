...
- **2단계 확인 플로우와 undo 저널.** 현재 프롬프트는 확인 질문을 금지하고
  ...
  가야 하는 작업이라 분리했습니다.
- **티어 교체의 비용·품질 효과 미측정.** 모델 ID(`gpt-5.4` / `gpt-5.4-nano`)는 실제
  키로 검증했고 라우팅 품질도 측정했지만(위 2항), 교체 *전후*를 같은 조건에서 비교한
  수치는 없습니다. 교체 전 구성으로 한 번 더 돌려야 나옵니다.
...
- **287 tests**, 19개 모듈. 외부 의존성(DB·OpenAI·Redis·S3) 전부 mock — 인프라 없이 실행
