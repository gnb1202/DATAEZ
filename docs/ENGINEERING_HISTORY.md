...
2026-09-10 문서 정리에서 이전 README의 관측성·라우팅·검색 개선 기록을 옮겼다. 아래 도구 수, 모델 비교, 평가 수치와 “현재”·“남은 작업”은 **당시 버전의 기록**이다. 현재 구현 상태는 [PRD](PRD.md), 검증 결과는 [문서 목차](README.md)를 따른다.

비동기 검색 색인, 트랜잭션 기반 파일 반영, 검색 및 자연어 통합 평가는 후속 단계에서 추가됐다. 이전 라우팅 평가 수치를 현재 제품의 전체 정확도로 해석하지 않는다.
