...
아래 표는 2026-09-12 포트폴리오 제작 당시 기록이다. 이후 09-13에는 [24문항 품질 회귀](../AGENT_QUALITY_PIPELINE.md)와 [대화 품질 관측·관리자 검토](../CHAT_QUALITY_OBSERVABILITY.md)를 추가했다. 당시의 통과 수·화면·영상은 새 버전 전체 검증으로 바꾸어 해석하지 않는다.
...
[리허설·검사·배포 근거](ACCEPTANCE.md), [촬영 기록](phase-3-capture.json), [영상 검수](VIDEO_RELEASE.md)를 연결했다. 당시 공개 리허설에서 사용한 영속 메시지 API는 토큰 사용량을 제공하지 않아 미확인 값을 `null`로 남겼다. 이를 0토큰이나 무료 호출로 해석하지 않는다. 과거의 다른 평가 결과는 [문서 인덱스](../README.md#검증-근거)에서 데이터셋과 실행 범위를 따로 확인할 수 있다.
