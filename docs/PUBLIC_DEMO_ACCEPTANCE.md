...
이번 단계까지의 배포는 **로컬 변경사항을 Vercel CLI로 올린 상태**다. Git 커밋·원격 push는 아직 수행하지 않았다. 다음은 누적 변경사항을 검토·커밋해 원격 저장소와 배포 소스를 일치시키고, 포트폴리오 설명·시연 자료를 정리하는 것이다. 사용자 관찰·실제 PG 파일·부하 검사는 별도 범위다.
