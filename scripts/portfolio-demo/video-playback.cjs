...
async function main(){
 ...
 const results=await Promise.all(['product','technical','loop'].map(async name=>{
  const page=await context.newPage();await page.goto(process.env.VIDEO_PREVIEW_URL || 'http://127.0.0.1:3134/');
 ...
 await fs.writeFile(process.env.VIDEO_PLAYBACK_REPORT || 'docs/portfolio-demo/phase-3-playback.json',JSON.stringify({passed:results.every(r=>r.ended&&r.width===1920&&r.height===1080),rate:1,results},null,2)+'\n');
 await browser.close();
}
