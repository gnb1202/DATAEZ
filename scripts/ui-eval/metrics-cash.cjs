...
(async () => {
  ...
  try {
    ...
    const chatReview = page.locator('#workspace-chat').getByRole('region', { name: '현금 입력 반영 확인', exact: true }).last();
    await chatReview.getByRole('button', { name: '반영 확정', exact: true }).waitFor();
    const link = page.locator('#workspace-chat').getByRole('link', { name: '현금 입력 검토하기', exact: true }).last();
    await link.waitFor();
    const href = await link.getAttribute('href');
    const id = new URL(href, ui).searchParams.get('cash_draft');
    await chatReview.getByText('30,000원', { exact: true }).waitFor();
    await chatReview.getByText('점심 현금 매출', { exact: true }).waitFor();
    await page.screenshot({ path: path.join(out, 'cash-draft.png'), fullPage: true });
    let pending = response(`/cash-entries/${id}/commit`);
    await chatReview.getByRole('button', { name: '반영 확정', exact: true }).click();
    report.first = await json(pending);
    assert.equal(report.first.status, 'committed');
    await chatReview.getByText(/반영 완료/).waitFor();
    await link.click();
    const cash = page.getByRole('region', { name: '현금 직접 입력' });
    await cash.getByText(/현금 장부에 반영했습니다/).waitFor();
