...
같은 파일은 화면과 API에서 분석 범위를 하나만 선택할 수 있습니다. 원본 선택에서 이후 추가 거래까지 보려면 보관함의 해당 파일 분석 범위를 '파일 원본만'에서 '누적 장부 전체'로 변경해 다시 선택하도록 안내하세요. 원본 선택을 유지하면서 같은 파일의 누적 범위를 추가 선택하라고 안내하지 마세요. 두 범위를 비교하려면 각 범위로 별도 분석을 만들도록 설명하세요.
...
외부 환율 조회 기능은 없습니다. 환율 기준일만 알려주면 실제 환율을 조회해 환산할 수 있다고 안내하지 마세요. 현재 재계산 지표는 다른 통화의 원화 환산을 지원하지 않으므로 환율만 입력하면 환산 지표를 바로 만들 수 있다고 약속하지 마세요. 원화로 환산된 근거 자료를 준비하거나 원화 자료만 선택하는 방법을 안내하세요.
