...
2026-09-09: Spoqa Han Sans Neo + 차콜/블루. 시안 공통 토큰과 차트를 적용했으며, 실제 Next.js 앱의 색상·테마 통합은 다음 페이즈다. 주요 계열은 블루, 비교 계열은 앰버, 취소는 로즈, 분석 가능 상태는 그린으로 표시한다.
...
실제 LLM, 로그인, 장부 SQL, Supabase Storage에는 연결하지 않았다. 앱의 Recharts와 의존성 파일도 변경하지 않았다. 채팅은 정해진 예시에 반응하고, 저장 상태는 새로고침 시 초기화한다. '새 분석'은 현재 샘플 분석을 여는 동작이다. 배치 드래그·리사이징은 이번 시안에 포함하지 않았다.

선택한 테마와 명시적으로 적용한 글꼴 설정을 브라우저 저장소에 보관한다. 저장된 설정이 없는 첫 방문은 다크로 시작한다. 글꼴 비교실과 작업 공간은 확정한 Spoqa Han Sans Neo로 시작하며 단순 비교는 저장하지 않는다. 이전 글꼴 설정은 Spoqa로 전환하고 크기·간격은 유지한다. 테마 상세는 [컬러 팔레트 설계](../../docs/COLOR_PALETTE.md), 글꼴 출처·숫자 정렬·비교 방법은 [글꼴 설계](../../docs/TYPOGRAPHY.md)에 기록한다. Spoqa 기본 글꼴은 실제 앱에도 적용했으며, 새 배치와 테마 통합은 후속 범위다.
